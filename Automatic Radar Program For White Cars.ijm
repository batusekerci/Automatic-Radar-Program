...
macro "AutoRadar v3 - Smart Tracking" {
    print("\\Clear");
    ...
    print("White Threshold: " + whiteThreshold);
    print("Particle size: " + minSize + "-" + maxSize + " px²");
    print("Min valid frames: " + minValidFrames);
    print("");
    
    runManualThresholdAnalysis(videoID, startFrame, endFrame, frameStep, whiteThreshold, minSize, maxSize);
}

function runManualThresholdAnalysis(videoID, startFrame, endFrame, frameStep, threshold, minSize, maxSize) {
    vehicleCount = 0;
    run("Clear Results");
                ...
                if (prevX.length > 0 && frame > startFrame) {
                    for (j = 0; j < prevX.length; j++) {
                        if (prevID[j] == currID[i]) {
                            ...
                            if (timeInterval > 0) {
                                speed = (realDist / timeInterval) * 3.6; // m/s to km/h
...
function calculateSummaryStatistics() {
    ...
    for (i = 0; i < vehicleIDs.length; i++) {
        for (j = i + 1; j < vehicleIDs.length; j++) {
            ...
            if (frameGap < -3 || frameGap > 3) continue;
            
            dx = idLastX[i] - idFirstX[j];
            dy = idLastY[i] - idFirstY[j];
            spatialDist = sqrt(dx*dx + dy*dy);
    ...
    for (i = 0; i < vehicleIDs.length; i++) {
        ...
        if (count > 0) {
            ...
            if (count >= minValidFrames && pathDistPx >= 200) {
                validVehicleCount++;
